#!/usr/bin/env python3
"""
Test LightGBM Ranker on competition test cases
"""
...
try:
    ...
    print("⚠️  LightGBM not available. Please install: pip install lightgbm")
    sys.exit(1)

def test_ranker():
    """Test ranking model on first 20 test cases."""
    
    print("="*80)
    print("LIGHTGBM RANKER VALIDATION")
    print("="*80)
    
    ranker = F1RankerPredictor()
    
    try:
        ranker.load_model("solution2/lightgbm_ranker.pkl")
        print("\n✓ Loaded trained ranking model")
    except FileNotFoundError:
        print("\n⚠️  No trained model found!")
        print(f"First run: python solution2\\lightgbm_ranker.py")
        return
    ...
    for i in range(1, 21):
        ...
        predicted_order = ranker.predict(test_data)
    ...
    improvement = avg_total - 12.9  # vs GradientBoosting at 64.5%
    
    if avg_total >= 16:
        print(f"\n🎉 EXCELLENT! Ranking achieves {avg_total/20*100:.1f}% accuracy")
        print(f"   Improvement over regression: +{improvement:.1f}%")
        print(f"   READY FOR COMPETITION SUBMISSION!")
    elif avg_total >= 14:
        print(f"\n✓ VERY GOOD! Ranking at {avg_total/20*100:.1f}% accuracy")
        print(f"   Improvement: +{improvement:.1f}%")
        print(f"   Close to target - consider ensemble for final push")
    elif avg_total >= 12:
        print(f"\n⚠️  MODERATE - {avg_total/20*100:.1f}% accuracy")
        print(f"   Change: +{improvement:.1f}%")
        print(f"   May need feature engineering or ensemble")
    else:
        print(f"\n❌ LOW ACCURACY - {avg_total/20*100:.1f}%")
        print(f"   Regression may be better for this dataset")
