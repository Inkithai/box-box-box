#!/usr/bin/env python3
"""
Box Box Box - F1 Race Simulator (Solution 2 - Optimized)

Using the same approach as Solution 1 but with cleaner code structure.
"""
...
# Default parameters - matching Solution 1
...
def load_params(path: str = "solution2/params.json") -> TireParams:
...
def parse_race_config(d: Dict[str, Any]) -> RaceConfig:
    """Parse race configuration from JSON"""
    return RaceConfig(
        track=str(d["track"]),
...
def parse_strategies(strategies: Dict[str, Any]) -> List[DriverPlan]:
    """Parse driver strategies from JSON"""
...
def simulate_total_time(race: RaceConfig, plan: DriverPlan, params: TireParams) -> float:
    """Simulate complete race and return total time"""
